      ...
      */
      /**
 * Add a marker on the Earth at a given latitude and longitude.
 */
      function addMarker(latitude, longitude, radius) {
        // Marker texture (could be a small pin or icon)
        var textureLoader = new THREE.TextureLoader();
        
        var markerTexture = textureLoader.load('/public/images/1729857349021-PicMan.jfif')
    
        // Create the sprite material with the marker texture
        ...
        // Create the sprite (2D object)
        var marker = new THREE.Sprite(material);
    
        // Convert latitude and longitude to 3D coordinates on Earth
        var phi = (90 - latitude) * (Math.PI / 180);  // Convert latitude
        var theta = (longitude + 180) * (Math.PI / 180);  // Convert longitude
    
        // Calculate the 3D position on the Earth's surface
        marker.position.x = radius * Math.sin(phi) * Math.cos(theta);
        marker.position.y = radius * Math.cos(phi);  // Position the marker at the Earth's surface
        marker.position.z = radius * Math.sin(phi) * Math.sin(theta);
    
        // Add the marker to the sphere cloud (or the scene directly)
        sphereCloud.add(marker);
    }
    
    
  
  /**
   * Add markers at different locations on Earth
   */
  function addMarkers() {
    // Example locations: San Francisco, Paris, Tokyo
    addMarker(37.7749, -122.4194, earthRadius); // San Francisco
    addMarker(48.8566, 2.3522, earthRadius); // Paris
    addMarker(35.6895, 139.6917, earthRadius); // Tokyo
    addMarker(-33.8688, 151.2093, earthRadius); // Sydney
    addMarker(51.5074, -0.1278, earthRadius); // London
  }
  ...
  addMarkers();
      ...
      */
      ...
      function addSkybox() {
        ...
        var skybox = new THREE.Mesh( geometry);
